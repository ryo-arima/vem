...
#[command(long_about = "VEM is a command-line tool written in Rust for efficiently managing multiple Vim environments. Switch between different .vim configurations easily based on your needs and preferences.")]
pub struct Cli {
    #[command(subcommand)]
...
pub fn handle_command(command: Commands) -> Result<()> {
    ...
    match command {
        Commands::Create { name, description: _ } => {
        ...
        Commands::Current => {
            match env_manager.get_current_environment() {
        ...
        Commands::Remove { name, force } => {
            if !force {
                print!("Are you sure you want to remove environment '{}'? (y/N): ", name);
